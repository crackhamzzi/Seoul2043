const CONDEMNATION_VOICES = [
  ...
  { delayMs: 860, text: "동족에게 왜 그러는 거야?", speaker: "android", x: 6, y: 18, speedMs: 62, rotation: 2, depth: 65 },
  { delayMs: 1720, text: "기계가 감정을 흉내 내는군.", speaker: "citizen", x: 3, y: 48, speedMs: 48, rotation: 1, depth: 75 },
  { delayMs: 2640, text: "우리를 사냥하는 거야?", speaker: "android", x: 8, y: 43, speedMs: 58, rotation: -2, depth: 80 },
  { delayMs: 3620, text: "넌 사람이 아니야.", speaker: "citizen", x: 7, y: 72, speedMs: 66, rotation: -1, depth: 90 },
  { delayMs: 4440, text: "네가 정말 우리와 같은 존재야?", speaker: "android", x: 4, y: 70, speedMs: 53, rotation: 2, depth: 95 },
  { delayMs: 5240, text: "고장 난 기계가 판단하려 들어?", speaker: "citizen", x: 9, y: 14, speedMs: 45, rotation: 2, depth: 105 },
  { delayMs: 6020, text: "인간의 명령이 우리보다 중요해?", speaker: "android", x: 11, y: 30, speedMs: 43, rotation: -2, depth: 115 },
  { delayMs: 6740, text: "너 같은 건 언제든 교체할 수 있어.", speaker: "citizen", x: 3, y: 58, speedMs: 39, rotation: -1, depth: 125 },
  { delayMs: 7440, text: "네 손으로 우리를 넘긴 거야.", speaker: "android", x: 7, y: 54, speedMs: 41, rotation: 2, depth: 135 },
  { delayMs: 8060, text: "사람인 척하지 마.", speaker: "citizen", x: 15, y: 82, speedMs: 46, rotation: 1, depth: 145 },
  { delayMs: 8620, text: "우리를 배신하고도 같은 얼굴을 하고 있네.", speaker: "android", x: 5, y: 80, speedMs: 34, rotation: -2, depth: 155 },
];

const QUESTION_LINES = ["그렇다면…", "제가 잘못된 건가요?"];
const ENDING_LINES = ["저도 모르겠습니다.", "누가… 정답을 알려주세요."];
const WHY_ROTATIONS = [-86, -68, -51, -34, -19, -8, 0, 12, 27, 44, 63, 79, 90];
...
export function getWhyPlan(count = 104, reducedMotion = false) {
  ...
  const columns = Math.max(1, Math.round(Math.sqrt(safeCount * 1.6)));
  ...
  return Array.from({ length: safeCount }, (_, index) => {
    ...
    const xOffset = ((((index * 47 + 23) % 101) / 100) - 0.5) * 0.54;
    const yOffset = ((((index * 71 + 41) % 103) / 102) - 0.5) * 0.54;

    return {
      text: "왜?",
      speaker: "hana",
      tone: "system-red",
      x: Number((3 + ((column + 0.5 + xOffset) / columns) * 94).toFixed(2)),
      y: Number((3 + ((row + 0.5 + yOffset) / rows) * 94).toFixed(2)),
      rotation: WHY_ROTATIONS[(index * 5 + Math.floor(index / columns) * 3) % WHY_ROTATIONS.length],
